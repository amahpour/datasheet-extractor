...
def derive_description(figure: Figure, ocr_mode: str) -> Figure:
    ...
    if figure.classification.type in {"plot", "timing_diagram", "block_diagram", "state_machine"} and confidence < 0.7:
        notes = "LLM recommended"
    else:
        notes = "none"

    figure.derived = Derived(description=Description(text=text, confidence=confidence, notes=notes))
    return figure
