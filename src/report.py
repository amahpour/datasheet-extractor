...
def _recommend_action(figure: Figure) -> tuple[str, str, float]:
    ...
    if fig_type == "plot":
        return "LLM: convert plot to CSV", "plot digitization requires semantic understanding", 0.8
    if fig_type in {"block_diagram", "state_machine", "timing_diagram"}:
        return "LLM: convert diagram to ASCII/Mermaid", "diagram interpretation is not reliable locally", 0.8
